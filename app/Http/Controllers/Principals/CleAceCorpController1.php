...
class CleAceCorpController extends Controller
{
    public static $PRINCIPAL_CODE = 'cle_ace_corp';
                ...
                foreach ($fileContentLines as $fileContentLine) {
                    ...
                    if ($lineCount > 1) {
                        ...
                        if (count($arrFileContentLine) > 1) {
                            ...
                            $conversion_qty = $conversion_qty=='' ? 0 : $conversion_qty;

                            $cp_cases =
                                str_replace(
                                    ',','',trim(str_replace('"', '', $arrFileContentLine[4]))
                                );
                            $cp_cases = $cp_cases=='' ? 0 : $cp_cases;

                            $cp_pcs =
                                str_replace(
                                    ',','',trim(str_replace('"', '', $arrFileContentLine[5]))
                                );
                            $cp_pcs = $cp_pcs=='' ? 0 : $cp_pcs;

                            $cp_panel_cases =
                                str_replace(
                                    ',','',trim(str_replace('"', '', $arrFileContentLine[6]))
                                );
                            $cp_panel_cases = $cp_panel_cases=='' ? 0 : $cp_panel_cases;

                            $cp_panel_pcs =
                                str_replace(
                                    ',','',trim(str_replace('"', '', $arrFileContentLine[7]))
                                );
                            $cp_panel_pcs = $cp_panel_pcs=='' ? 0 : $cp_panel_pcs;
                            ...
                            $arrLines[] = [
                                ...
                                'cp_cases' => $cp_cases,
                                'cp_pcs' => $cp_pcs,
                                'cp_panel_cases' => $cp_panel_cases,
                                'cp_panel_pcs' => $cp_panel_pcs
                            ];
                        }
                    }
                    $lineCount++;
