    ...
    public function generateTemplatedData(Request $request)
    {
        ...
        try {
            ...
            $template_variation_count = DB::table(PrincipalsUtil::$TBL_PRINCIPALS)
                ->where('code', $this->PRINCIPAL_CODE)->pluck('template_variation_count')->first();
            // dd($template_variation_count);
            ...
            for ($tvc_index = 0; $tvc_index < $template_variation_count; $tvc_index++) {
                ...
                foreach ($pendingInvoices as $pendingInvoice) {
                    ...
                    GenerateTemplated::dispatch("Generating template $tvc_index ($progressPercent%)");
                    ...
                    if ($tvc_index == 0) {
                        ...
                        $arrGenerated = [
                            ...
                            'cf_dsp_name_value' =>      $settings['DSP_'. $pendingInvoice->group],
                    ...
                    if ($tvc_index == 1) {
                        ...
                        $return_indicator = $pendingInvoice->return_indicator ?? 'NA';

                        // Generated data line structure
                        if($return_indicator != 'NA') {
                            $arrGenerated = [
                                //commons
                                'customer_code' =>          $customer_code_supplier,
                                'alturas_customer_code' =>  $customer_code,
